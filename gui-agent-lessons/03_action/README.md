...
| 行动空间 | 形式 | 表达力 | 安全性 | 取舍 |
|---|---|---|---|---|
| ① **自由代码**（CodeAct 式） | LLM 输出 Python/JS 直接执行 | 最强 | 🚫 最差——浏览器场景能干任何事（跳转/提交/下载执行），一个恶意 prompt 就完蛋 | 适合沙箱完好的 Code Agent（第六门课 L06），不适合直接控浏览器 |
...
> 🎯 这和第六门课 Reflexion 的精神一致：失败不可怕，把失败的语言化反馈喂回去，下一轮就能改进。本课把它用在动作校验上——非法动作 = 一次小失败，错误信息 = 语言化反馈。
