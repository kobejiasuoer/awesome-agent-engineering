...
### ClawBench：真实网站上的外部效度

本课的本地 mini-benchmark 适合回归和消融，但不能回答 agent 面对持续变化的线上网站是否仍能完成任务。[ClawBench](https://claw-bench.com/)（[论文](https://arxiv.org/abs/2604.08523) · [代码](https://github.com/TIGER-AI-Lab/ClawBench)）用 V1/V2 两套共 283 个任务直接评估真实网站上的浏览器 agent；每次运行隔离在容器中，并同步记录屏幕录像、动作截图、HTTP 流量、浏览器动作和 agent 消息五层证据。任务完成请求在发往目标服务前被拦截并按任务规则核验，使成功判定与失败复盘都能回到可检查的证据。

两类基准不是替代关系：本地 WebArena 思路用于稳定 CI、机制回归和消融；ClawBench 用于检查真实网站变化下的外部效度。先用前者定位回归，再用后者验证「今天的线上环境里是否做得成」。
